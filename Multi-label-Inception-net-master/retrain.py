...
tf.app.flags.DEFINE_float('threshold', 0.00,
                          """threshold.""")
#测试所占的百分比
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
  ...
  with tf.name_scope('cross_entropy'):
    cross_entropy = tf.nn.sigmoid_cross_entropy_with_logits(
      labels = logits, logits = ground_truth_input)
  ...
  with tf.name_scope('accuracy'):
    with tf.name_scope('correct_prediction'):
     ...
     # print(sess.run(result_tensor)) 
      #print(sess.run(ground_truth_tensor)) threshold
      y_p = tf.round(result_tensor - FLAGS.threshold)
      y_t = ground_truth_tensor
      ...
      #correct_prediction = y_p * y_t
      #correct_prediction = tf.equal(tf.round(result_tensor - FLAGS.threshold), ground_truth_tensor) 
    with tf.name_scope('accuracy'):
      # Mean accuracy over all labels:
      # http://stackoverflow.com/questions/37746670/tensorflow-multi-label-accuracy-calculation
      ###
       evaluation_step  = tf.reduce_mean(F1_scores)
...
def main(_):
  ...
  for i in range(FLAGS.how_many_training_steps):
    ...
    if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
      train_accuracy, cross_entropy_value = sess.run(
          ...
          feed_dict={bottleneck_input: train_bottlenecks,
                     ground_truth_input: train_ground_truth})
      print('%s: Step %d: Train f1_score = %.5f' % (datetime.now(), i,
                                                      train_accuracy ))
      ...
      print('%s: Step %d: Validation f1_score = %.5f' %
  ...
  print('Final f1_score accuracy = %.5f' % (test_accuracy))
